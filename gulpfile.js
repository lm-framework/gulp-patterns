var args = require('yargs').argv;//接收参数,保存了用户输入信息
...
gulp.task('templatecache', ['clean-code'], function() {
    ...
    return gulp
        ...
        .pipe($.if(args.verbose, $.bytediff.start()))//在构建文件过程前后比较文件大小
...
function inject(src, label, order) {
    ...
    if (label) {
        options.name = 'inject:' + label;//将文件注入到label 定义的地方!
    }
...
function runNodeInspector() {
    ...
    var exec = require('child_process').exec;//新建子进程
    exec('node-inspector');
}
